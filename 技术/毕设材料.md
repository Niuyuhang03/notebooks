...
这个embedding的过程，就是知识表示学习，就是这篇论文所研究的对象

==next==

目前的知识表示学习学习主要有四种方法，分别是

其中最新的GAT模型，即Graph attention network模型，是利用了注意力机制，将实体和关系表示成向量。

但GAT模型的注意力权重只用到了图谱的实体，没有充分利用图谱的信息。

==next==

因此我们针对GAT模型的缺陷，提出了一个使用多种注意力权重的网络R-GAT，并投入到实体分类和链接预测任务中。
...
gpu跑rwr_f4,CnF4,CmF4,DiF4,rf4，要跑rwrw4
