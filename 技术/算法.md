...
+ 从文本中匹配指定字符串，暴力求解时，需要对文本每个字母作为开头字母匹配。实际上，如果部分匹配成功，其余部分匹配不成功，匹配成功的部分中并不是每个字母都需要再作为开头字母去检查。KMP算法需要一个list，长度等于被匹配字符串。被匹配字符串abcdabd的从前往后的所有字串a、ab、abc、abcd、abcda、abcdab、abcdabd，对于第i个字串，找出其所有前缀和后缀的共有元素个数记为list[i]，如abcdab的前缀为['a','ab','abc','abcd','abcda']，后缀为['b','ab','dab','cdab','bcdab']，共同元素为'ab'，因此list[5]=1，得到list=[0,0,0,0,1,2,0]。对于每次匹配不成功，首字母后移=已经匹配成功长度k-list[k-1]
+ 实质为，如果abcdab已经匹配成功，最后一位d未成功，只需后移6-2=4位，即原本匹配到被匹配文本第二个a的位置，下一次匹配到了被匹配文本首字母a
...
+ 《剑指offer》的乱序讲解版，讲解不细，基本为串讲，有对应在线练习题，不讲解正确代码，但难度有，适合科班学过，准备面试用。
