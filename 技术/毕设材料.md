...
首先是研究的背景和意义。当我们遇到问题时，我们习惯于在搜索引擎中去寻找答案。

如果我们想知道“梅西的故事”，google会将所有结果都展示出来，供我们筛选。

但如果我们想知道“梅西的国籍”这样简单准确的答案，。。不够好。。

==click==

现在的搜索引擎是怎么做的呢？当我们搜索具有简单准确的答案的问题时，会直接返回几个query box。。。包括问题的答案和实体的信息。这就是通过知识图谱实现的

==next==

知识图谱的结构是一张有向图，包括实体和实体之间的关系。知识图谱中最重要的结构就是<>三元组。想要研究和利用知识图谱
