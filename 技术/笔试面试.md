...
+ 美团推荐算法：没考机器学习。5个题，简单题用dp会卡超时。。。。排列组合考了两个题，暂时不会算。图题复杂。
  + 一面：问最近项目，问很多。python 多线程 yield 函数里参数前加* ，我提到java写过多线程。过拟合，正则化，梯度延迟，梯度爆炸，梯度消失，优化函数adam。java关键字violate（类似）。写了一个排序，给成绩排序，我写的计数排序，不过可能稍有问题。一个月后感谢信，三天后被捞
  + 二面：项目，l1l2正则化，为什么if []是false，工业推荐的过程，快排，链表快排
