...
4.21：更新fb数据，未分类数据给了film类别。复制到各个项目中，rgcn运行数据失败，暂未处理。服务器在运行adsf，gpu占用率没有niuyh，且没生成文件。
...
adsf慢在citeseer上25小时跑了144个epoch，71.2%，使用了gpu但使用率多为0，怀疑是cpu读取数据时间远大于gpu计算时间。输出时间debug